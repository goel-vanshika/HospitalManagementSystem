...
def medical_records():
    ...
    if result:
        Cross_Consulation_department = result[0][7]
        new_Cross_Consulation_department = st.text_input("Cross Consulation Department:", Cross_Consulation_department)

    if st.button("Update Patient Details"):
        edit_details_mr(new_Cross_Consulation_department, selected_patient)
        st.success("Successfully updated: {}".format(selected_patient))
        

def view_all_data_patient(patient_id):
    c.execute('SELECT * FROM Medical_records WHERE Patient_id="{}"'.format(patient_id))
    data = c.fetchall()
    return data

def view_all_data_doctor(doctor_id):
    c.execute('SELECT * FROM Medical_records WHERE Doctor_id="{}"'.format(doctor_id))
    data = c.fetchall()
    return data

def view_all_data_department(department):
    c.execute('SELECT * FROM Medical_records WHERE Department="{}"'.format(department))
    data = c.fetchall()
    return data

def edit_details_mr(new_Cross_Consulation_department, selected_patient):
    query= "UPDATE Medical_records SET Cross_Consulation_department=%s WHERE Patient_name=%s"
    c.execute(query,(new_Cross_Consulation_department,selected_patient))
    mydb.commit()
    data = c.fetchall()
    return data

def view_only_patient_names():
    c.execute('SELECT Patient_name FROM Medical_records')
    data = c.fetchall()
    return data

def get_details(Patient_name):
     c.execute('SELECT * FROM Medical_records WHERE Patient_name="{}"'.format(Patient_name))
     data = c.fetchall()
     return data

if __name__ == '__medical_records__':
    medical_records()
